...
pub fn validate_uuid(value: &str, what: &str) -> Result<(), String> {
    ...
    if well_formed {
        ...
        Err(format!("{what} '{value}' is not a valid UUID"))
    }
}
...
pub fn uuid_arg(value: &str) -> String {
    format!("[{}]", to_u128(value))
}
...
pub fn uuid_opt_arg(value: Option<&str>) -> String {
    match value {
...
pub fn uuid_vec_arg<S: AsRef<str>>(values: &[S]) -> String {
    let items: Vec<String> = values.iter().map(|v| uuid_arg(v.as_ref())).collect();
    format!("[{}]", items.join(","))
}

fn to_u128(value: &str) -> u128 {
    let hex: String = value.chars().filter(|c| *c != '-').collect();
    u128::from_str_radix(&hex, 16).expect("value must already be a validated canonical UUID")
}
...
mod tests {
    ...
    fn renders_uuid_args() {
        assert_eq!(uuid_arg("fce40ba8-cab8-42d1-acd5-be8aed7b53e0"), "[336149535101120413114579957789750940640]");
        assert_eq!(uuid_opt_arg(None), "null");
        assert_eq!(
            uuid_opt_arg(Some("fce40ba8-cab8-42d1-acd5-be8aed7b53e0")),
            r#"{"some":[336149535101120413114579957789750940640]}"#
        );
        assert_eq!(uuid_vec_arg::<&str>(&[]), "[]");
        assert_eq!(
            uuid_vec_arg(&["fce40ba8-cab8-42d1-acd5-be8aed7b53e0", "fce40ba8-cab8-42d1-acd5-be8aed7b53e0"]),

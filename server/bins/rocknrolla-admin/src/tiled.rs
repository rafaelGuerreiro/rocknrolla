...
pub fn parse_level(source: &str) -> Result<ImportedLevel, String> {
    ...
    let reward_lootbox_id = string_prop(&map.properties, "reward_lootbox_id")?.unwrap_or_default();
    ...
    for successor in &successors {
        if *successor == id {
            return Err(format!("level '{id}' lists itself as a successor"));
        }
    }
...
mod tests {
    ...
    fn sample(gameplay_z: u64, extra_flags: u32) -> String {
        ...
        serde_json::json!({
            ...
            "tilesets": [{"firstgid": 1}],
            "properties": [
                {"name": "id", "type": "string", "value": "test-level"},
                {"name": "name", "type": "string", "value": "Test Level"},
                {"name": "starting", "type": "bool", "value": true},
                {"name": "successors", "type": "string", "value": "next-level, other"}
            ],
            "layers": [{
    ...
    fn parses_a_valid_map() {
        ...
        assert_eq!(level.id, "test-level");
        assert!(level.is_starting);
        assert_eq!(level.successors, vec!["next-level", "other"]);

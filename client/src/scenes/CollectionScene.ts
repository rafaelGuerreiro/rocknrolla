...
export class CollectionScene extends Phaser.Scene {
  ...
  private openLootbox(playerLootboxId: Uuid): void {
    ...
    const failed = (error: unknown) => {
      ...
      note(
        this,
        200,
        `Could not open the lootbox: ${error instanceof Error ? error.message : error}`,
      );
    };
    const onUpdate = (
      _ctx: unknown,
      ...
      if (
        row.id.compareTo(playerLootboxId) !== 0 ||
        !row.opened ||
        !row.awardedPieceId
      )
        return;
  ...
  private drawCollection(): void {
    ...
    const counts = new Map(
      [...conn.db.vw_my_piece.iter()].map((row) => [
        row.pieceId.toString(),
        row.count,
      ]),
    );
    ...
    for (const character of characters) {
      ...
      this.add
        .text(
          150,
          y,
          `${character.name} (${isUnlocked ? 'unlocked' : 'locked'})`,
          {
            fontFamily: UI_FONT,
            fontSize: '22px',
      ...
      const line = pieces
        ...
        .map(
          (piece) => `${piece.name} ×${counts.get(piece.id.toString()) ?? 0}`,
        )
        .join('   ');
      this.add

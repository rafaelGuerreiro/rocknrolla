...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const eligible = characters.filter((character) => unlocked.has(character.id.toString()));
    ...
    if (eligible.length === 0) {
      note(this, this.scale.height / 2, 'No unlocked characters. Collect pieces to unlock one.');
    }
    ...
    characters.forEach((character, index) => {
      ...
      const required = pieces.filter((piece) => piece.characterId.toString() === characterId);
      const have = required.filter((piece) => owned.has(piece.id.toString())).length;
      ...
      button(this, x, y + 36, character.name, () => this.startRun(characterId), {
        width: 220,
        disabled: !isUnlocked,
      });
    });

    note(this, 110, 'Stats come from the server and change how the character rolls, jumps, floats, and burns.');
